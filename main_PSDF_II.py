...
def change_VNF_priority(state: State):
    ...
    physical_node_list = state_copy.get_PN_list()
    physical_cpu_tuple = [(pn.get_total_cpu() - pn.get_rest_cpu(), pn.get_PN_ID()) for pn in physical_node_list]
    vnf_list = state_copy.get_VNF_list()
    total_vnf_req = sum([vnf.get_req_cpu() for vnf in vnf_list])
    PN_need_num = math.ceil(total_vnf_req / available_cpu_per_node)
    standard = total_vnf_req / PN_need_num
    delta_cpu_tuple = [(x - standard, y) for (x, y) in physical_cpu_tuple]
    delta_cpu_tuple_ordered = sorted(delta_cpu_tuple, key=lambda x: x[0], reverse=True)
    VNF_priority = []
    cont = 0
    while True:
        for (x, y) in delta_cpu_tuple_ordered:
            pn_temp = None
            for pn in physical_node_list:
                if pn.get_PN_ID() == y:
                    pn_temp = pn
                    break
            pn_vnf_list = pn_temp.get_vnf_list()
            delta_vnf_list = []
            for m in pn_vnf_list:
                if x >= 0:
                    temp = -abs(m.get_req_cpu() - x)
                else:
                    temp = m.get_req_cpu() - x
                delta_vnf_list.append(temp)
            if len(delta_vnf_list) == 0:
                continue
            max_index = delta_vnf_list.index(max(delta_vnf_list))
            vnf_ID = pn_vnf_list[max_index].get_vnf_ID()
            del pn_vnf_list[max_index]
            VNF_priority.append(vnf_ID)
            cont += 1
            if cont == VNF_num:
                break
        if cont == VNF_num:
            break
    print('vnf priority', VNF_priority)
    return VNF_priority
    ...
    for eps in range(episode):
        ...
        for time_slot in range(training_stop_time):
            ...
            for i in Priority:
                ...
                if overload_flag and min(
                        cost_list) > Parameters.penalty + Parameters.beta1 * V + Parameters.basic_energy:
        ...
        cost.to_csv('./result/PSDF_II_result_PN' + str(PN_num) + '_VN' +
                    str(VNF_num) + '_episode' + str(eps) + '.csv', index=False)

        tot_RL.plot_cost('PSDF_II_PN' + str(PN_num) + '_VN' + str(VNF_num) + '_episode' + str(eps))
