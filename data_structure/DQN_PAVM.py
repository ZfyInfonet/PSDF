...
class DeepQNetworkPriorityAware:
    ...
    def store_transition(self, s, a, r, s_, is_end):
        is_end = 1 - is_end
        transition = np.hstack((s, a, r, is_end, s_))
    ...
    def learn(self):
        ...
        q_target[batch_index, eval_act_index] = \
            reward + self.gamma * np.max(q_next, axis=1) * batch_memory[:, self.n_features + 2]
    ...
    def plot_cost(self, name: str):
        import matplotlib.pyplot as plt

        plt.plot(np.arange(len(self.cost_his)), self.cost_his)
        plt.ylabel('Loss')
        plt.xlabel('training steps')
        plt.savefig('./net_loss_result/figures/' + name + '.pdf')
        cost_pd = pd.DataFrame(self.cost_his)
        cost_pd.to_csv('./net_loss_result/' + name + '_loss.csv', index=True)
